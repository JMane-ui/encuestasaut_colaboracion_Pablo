...
class Surveys extends CI_Controller{
    ...
    public function index()
    {
        ...
        $this->load->view('templates/header', $data, array('style' => 'dashboard'));
        $this->load->view('es/app/dashboard' , $data);
    ...
    public function view_survey($form_id)
    {
        ...
        $this->load->view('templates/header', $data , array('style' => 'view'));
        $this->load->view('es/app/view' , $data);
    ...
    public function share_form()
    {
        if ( empty($_POST) )
            ...
            $link = URLhost.'answer-survey/' . $uuid;
    }
    ...
    public function create_survey_view()
    {
        ...
        $this->load->view('templates/header', $data , array('style' => 'survey'));
        $this->load->view('es/app/survey' , $data);
    ...
    public function create_survey()
    {
        ...
        if ( array_key_exists( 'indexes' , $arr ) || array_key_exists( 'double' , $arr ) ){
            ...
            if ($check == 'false' ){ 
                ...
                $form_id = $this->survey_model->store_question_form($title,$description,$objective,$slug,$user_id,$survey_img);
                $arr['id'] = $form_id;
    ...
    public function update_survey()
    {
        ...
        if ( array_key_exists( 'indexes' , $arr ) || array_key_exists( 'double' , $arr ) ){
            ...
            $base = ( strpos($survey_img , 'surveys')+strlen('surveys') );
            $survey_img = substr( $survey_img , $base+1 , strlen($survey_img) );
            $this->survey_model->edit_question_form($form_id,$title,$description,$objective,$slug,$survey_img);
